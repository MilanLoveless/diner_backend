use super::super::types::{GameFormData, GameRecord};
use chrono::Utc;
use sqlx::PgPool;
use uuid::Uuid;
...
#[tracing::instrument(
    name = "Saving new game to the database",
    skip(name, description, link, pool)
)]
pub async fn insert(
    pool: &PgPool,
    name: &String,
...
#[tracing::instrument(name = "Updating existing game in the database", skip(form, pool))]
pub async fn update(pool: &PgPool, id: &Uuid, form: &GameFormData) -> Result<(), sqlx::Error> {
    sqlx::query!(
        r#"
